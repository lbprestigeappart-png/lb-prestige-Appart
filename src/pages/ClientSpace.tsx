...
type CartItem = { id: string; kind: "dish" | "drink"; name: string; price: number; qty: number; restaurantId: string };

function RestaurantsSection({
  restaurants, selectedId, setSelectedId, cart, setCart, cartOpen, setCartOpen,
}: {
  restaurants: any[];
  ...
  function buildWhatsappLink() {
    ...
    const lines = items.map(
      (i) => `- ${i.qty}x ${i.name} (${(i.price * i.qty).toLocaleString("fr-FR")} FCFA)`
    );
    ...
    <div className="grid gap-5 relative">
      ...
      <Card className="p-6 bg-gradient-to-br from-card via-card to-noir border-gold/30 shadow-gold relative overflow-hidden">
        ...
        <div className="relative">
          <div className="text-[10px] tracking-[0.3em] uppercase text-gold mb-2">Room Service & Restauration de luxe</div>
          <h2 className="font-display text-3xl text-gold-gradient leading-tight">La table à votre porte</h2>
          <p className="text-sm text-muted-foreground mt-2 max-w-xl">
            Une sélection de restaurants partenaires. Composez votre commande et envoyez-la directement par WhatsApp — livraison à l'appartement.
          </p>
        </div>
      </Card>
      ...
      {cartCount > 0 && (
        <button
          ...
          className="fixed bottom-6 right-6 z-40 gradient-gold text-noir rounded-full shadow-gold flex items-center gap-2 px-5 py-3 hover:scale-105 transition-transform"
        >
            ...
            {restaurants.map((r) => {
              ...
              return (
                <button
                  ...
                  className="group text-left rounded-lg overflow-hidden border border-border bg-card hover:border-gold/60 transition-all hover:-translate-y-0.5 hover:shadow-gold"
                >
                  ...
                  <div className="px-4 py-3 flex items-center justify-between border-t border-border">
                    ...
                    <span className="text-[10px] uppercase tracking-wider text-gold">Voir le menu →</span>
                  </div>
                </button>
              );
            })}
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-noir/80 backdrop-blur-sm" onClick={() => setCartOpen(false)}>
          <div className="w-full sm:max-w-md bg-card border border-gold/30 rounded-t-lg sm:rounded-lg shadow-gold max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              {Object.entries(cart).map(([key, item]) => (
                <div key={key} className="flex items-center justify-between gap-3 p-3 rounded-md border border-border bg-secondary/30">
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium truncate">{item.name}</div>
                    <div className="text-[11px] font-mono text-gold">{item.price.toLocaleString("fr-FR")} FCFA</div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => updateQty(key, -1)} className="w-7 h-7 rounded border border-border hover:border-gold flex items-center justify-center">
                    ...
                    <button onClick={() => removeItem(key)} className="ml-2 text-muted-foreground hover:text-destructive">
...
function RestaurantDetail({
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="text-xs text-muted-foreground hover:text-gold flex items-center gap-1.5">
      ...
      <MenuSection title="Plats Signatures" icon={UtensilsCrossed} items={dishes}
        kind="dish" cart={cart} addToCart={(i) => addToCart("dish", i, restaurant.id)} updateQty={updateQty} />

      <MenuSection title="Carte des Boissons" icon={Wine} items={drinks}
      ...
      {items.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {items.map((d: any) => {
            ...
            return (
              <div key={d.id} className="rounded-lg border border-border bg-card overflow-hidden flex hover:border-gold/40 transition-colors">
                ...
                <div className="flex-1 p-3 flex flex-col min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    {qty === 0 ? (
                      <button
                        onClick={() => addToCart(d)}
                        className="gradient-gold text-noir rounded-full w-8 h-8 flex items-center justify-center hover:scale-110 transition-transform"
                      ...
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => updateQty(key, -1)} className="w-7 h-7 rounded-full border border-gold/40 text-gold hover:bg-gold/10 flex items-center justify-center">
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="w-5 text-center text-sm font-mono">{qty}</span>
                        <button onClick={() => updateQty(key, 1)} className="w-7 h-7 rounded-full gradient-gold text-noir flex items-center justify-center">
                          <Plus className="w-3 h-3" />
                        </button>
                      </div>
                    )}
